  ...
  async function handleSave() {
    ...
    try {
      ...
      if (role === "maintenance") {
        ...
        payload.failureCount = (form.failureCount ?? material.failureCount ?? "0") as string;
      }
      ...
      setMaterial((prev) => (prev ? ({ ...prev, ...payload } as Material) : prev));
    } catch (err) {
      console.error(err);
      ...
      <div className="w-full max-w-4xl bg-white rounded-3xl shadow-xl overflow-hidden flex flex-col">
        ...
        <div className="p-4 sm:p-6 space-y-6 text-sm text-black overflow-y-auto max-h-[calc(100vh-6rem)]">
          ...
          <Section title="Manufacturer & Technical (Read-Only)">
            ...
            <TwoCol label="Manufacturing Date" value={material.manufacturingDate} />
            <TwoCol
              label="Expected Life"
          ...
          {role === "installation" && (
            <Section title="Installation Staff – Track & Depot Updates">
              ...
              <div className="flex flex-col gap-2 mt-2 sm:flex-row sm:items-center sm:justify-between">
                ...
                <select
                  value={form.installationStatus ?? material.installationStatus ?? "Not Installed"}
                  onChange={(e) =>
                ...
                <span
                  className={`px-3 py-1 rounded-full text-[11px] font-semibold w-fit ${
                    (form.installationStatus ?? material.installationStatus) === "Installed"
                      ? "bg-green-100 text-green-700"
                      : "bg-yellow-100 text-yellow-700"
                  }`}
                >
                  {form.installationStatus ?? material.installationStatus ?? "Not Installed"}
                </span>
              </div>
            </Section>
          )}
